...
class LlavaMetaForCausalLM(ABC):
    ...
    def encode_image_video_memory_batch(self, images_list, video_idx_in_batch, pg_scores=None):
        assert len(video_idx_in_batch)<2, "<<<encode_image_video_memory>>> i dont want to process multiple image or video"
        # print("images_list shape: ", images_list[0].shape)
        batch_size = 32
        concat_images = torch.cat([image for image in images_list], dim=0)
        ...
        if has_video:
            concat_videos = torch.cat([video.reshape(video.shape[0] // mm_local_num_frames, -1, video.shape[-1]) for video in concat_videos], dim=0)
            videos_features = self.get_model().mm_projector(concat_videos, local_num_frames=mm_local_num_frames, pg_scores=pg_scores)
    ...
    def prepare_inputs_labels_for_multimodal(self, input_ids, position_ids, attention_mask, past_key_values, labels, images, modalities=["image"], image_sizes=None, pg_scores=None):
        ...
        if type(images) is list or images.ndim == 5:
            ...
            if vision_encode_type == "image": # image backbone, process video by frame
                ...
                image_features = self.encode_image_video_memory_batch(images_list, video_idx_in_batch=video_idx_in_batch, pg_scores=pg_scores) 
            
            elif vision_encode_type == "video_image": # when single frame in UMT，video_idx_in_batch is []; when multi-frame，video_idx_in_batch is [0]
                image_features = self.encode_video_image(images_list, video_idx_in_batch=video_idx_in_batch)
            elif vision_encode_type == "video_image_memory": # when single frame in UMT，video_idx_in_batch is []; when multi-frame，video_idx_in_batch is [0]
            ...
            if mm_patch_merge_type == "flat":       #SigLIP version here
                # print("<<< Warning >>> Maybe I dont want to flat...")
                image_features = [x.flatten(0, 1) for x in image_features]
            elif mm_patch_merge_type.startswith("spatial"):  #UMT version here
