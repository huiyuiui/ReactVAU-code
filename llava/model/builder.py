...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, device_map="auto", attn_implementation="flash_attention_2", customized_config=None, overwrite_config=None, **kwargs):
    ...
    if "llava" in model_name.lower() or is_multimodal:
        # Load LLaVA model
        # Detect LoRA checkpoint: check both name and file-based detection
        # File-based detection covers checkpoints whose directory name doesn't contain "lora"
        _has_lora_in_name = "lora" in model_name.lower()
        _has_lora_files = os.path.exists(os.path.join(model_path, "adapter_config.json"))
        _is_lora = _has_lora_in_name or _has_lora_files
        if _has_lora_files and not _has_lora_in_name:
            rank0_print(f"Auto-detected LoRA checkpoint (adapter_config.json found in {model_path})")
        ...
        if _is_lora and model_base is not None:
            ...
            if "mixtral" in model_name.lower():
                ...
                lora_cfg_pretrained = LlavaMixtralConfig.from_pretrained(model_base)
                tokenizer = AutoTokenizer.from_pretrained(model_base, use_fast=False)
                ...
                lora_cfg_pretrained = LlavaGemmaConfig.from_pretrained(model_base)
                tokenizer = AutoTokenizer.from_pretrained(model_base, use_fast=False)
            ...
            elif "qwen" in model_name.lower() or "quyen" in model_name.lower() or "qwen" in model_base.lower():
            ...
            if not os.path.exists(non_lora_path):
                ...
                rank0_print(f"non_lora_trainables.bin not found in {model_path}, trying parent directory: {parent_dir}")
            
            # Always load non_lora_trainables to CPU first to minimize GPU peak
            # memory during LoRA merge. load_state_dict() will copy tensors to
            # the model's device and free the CPU copies.
            _non_lora_map = "cpu"
            if os.path.exists(non_lora_path):
                non_lora_trainables = torch.load(non_lora_path, map_location=_non_lora_map)
            else:
                ...
                def load_from_hf(repo_id, filename, subfolder=None):
                    ...
                    return torch.load(cache_file, map_location=_non_lora_map)
