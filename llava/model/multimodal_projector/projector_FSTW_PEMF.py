...
class ToMe_FSTW_PEMF(nn.Module):
    ...
    def forward(self, x, local_num_frames, is_image=False, pg_scores=None): # 單帧49
        ...
        # Training MemoryManager with PG-aware Anomaly Pool
        # SFTW=12 frames, Pool=6 frames, PEMF=2048 tokens
        # event_split_window=4 must match eval scripts for train-eval consistency
        memory_manager = MemoryManager(
            self.mm_hidden_size, self.num_attention_heads,
        ...
        for frame_idx in range(x.shape[0]):
            # Determine per-frame anomaly score from PG
            if has_pg_scores and frame_idx < len(pg_scores):
                score = float(pg_scores[frame_idx])
            else:
                score = 0.0

            # Use anomaly-score-aware update for APS protection
            memory_manager.update_with_anomaly_score(x[frame_idx], anomaly_score=score)

            # Add high-score frames to Anomaly Pool
            if score >= anomaly_pool_threshold:
                memory_manager.update_anomaly_pool(x[frame_idx], score)
        
        visual_tokens = memory_manager.get_memory_tokens()  # [1, N_Visual, C]
        
        x_out = self.mlp(visual_tokens)
