...
class LLaVATrainer(Trainer):
    ...
    # Override training_step to handle OOM
    def training_step(self, model, inputs, num_items_in_batch=None):
        ...
        try:
            ...
            # Handle gradient accumulation to ensure correct loss values
            if self.args.gradient_accumulation_steps > 1 and not self.deepspeed:
                # DeepSpeed handles loss scaling automatically, but pure Accelerate requires manual division
            ...
            rank0_print(f"\n{'!'*80}")
            rank0_print(f"[WARNING] CUDA OOM detected during training_step! Skipping this batch.")
            rank0_print(f"{'!'*80}\n")
            
            torch.cuda.empty_cache()
            gc.collect()
            
            # Return 0 loss to ensure Trainer internal logic continues
            return torch.tensor(0.0, device=self.args.device, requires_grad=True)
            
        except Exception as e:
            rank0_print(f"\n[WARNING] Unknown error during training_step: {e}. Skipping batch.")
            torch.cuda.empty_cache()
            gc.collect()
            return torch.tensor(0.0, device=self.args.device, requires_grad=True)
