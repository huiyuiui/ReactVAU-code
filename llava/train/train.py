    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "image" in self.list_data_dict[i]:
            ...
            if self.pg_scores_dict is not None:
                video_file = self.list_data_dict[i]["video"]
                # Try to find matching PG scores entry
                # video_file has data_root prepended; pg_scores_dict uses relative paths
                pg_entry = None
                for key in self.pg_scores_dict:
                    if video_file.endswith(key):
                        pg_entry = self.pg_scores_dict[key]
                        break
